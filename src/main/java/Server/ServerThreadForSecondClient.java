 ...
 */
public class ServerThreadForSecondClient extends Thread {
    private Socket socket;
    private SynchronousQueue<Game> queue;
    private Game game;

    public ServerThreadForSecondClient(Socket socket, SynchronousQueue<Game> queue, Game game) {
        this.game = game;
        this.queue = queue;
        this.socket = socket;
    }
    public void run() {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(socket.getInputStream()));
            ...
            int i = 0;
            while (true){
                System.out.println("Second client step N:" + i);
                try {
                    game = queue.take();
                }
